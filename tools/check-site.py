...
try:
 with sync_playwright() as p:
  ...
  expect(page.locator('html')).to_have_attribute('lang','en');page.locator('#copy').click();expect(page.locator('#copy-status')).to_have_text('Copied');assert page.evaluate('navigator.clipboard.readText()')=='pi install npm:pi-design-mode@0.2.2'
  page.locator('#language').click();expect(page.locator('html')).to_have_attribute('lang','zh-CN');expect(page.locator('h1')).to_contain_text('留在文件里');expect(page.locator('nav')).to_have_attribute('aria-label','主导航');expect(page.locator('video')).to_have_attribute('aria-label','真实界面操作演示');expect(page.locator('.installation')).to_have_attribute('aria-label','安装');page.screenshot(path=str(OUT/'zh-desktop.png'),full_page=True)
  page.locator('#language').click();page.locator('video').scroll_into_view_if_needed();page.evaluate("document.querySelector('video').play()");page.wait_for_timeout(1500);assert page.evaluate("document.querySelector('video').currentTime")>0;assert page.evaluate("Math.abs(document.querySelector('video').duration-62.708333)<0.1 && document.querySelector('video').videoWidth===1920 && document.querySelector('video').videoHeight===984"),'不是已批准的 TUI + HTML 视频';page.evaluate("document.querySelector('video').pause()");page.screenshot(path=str(OUT/'en-desktop.png'),full_page=True)
